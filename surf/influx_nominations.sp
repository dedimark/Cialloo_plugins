...
{
    char query[512];
    FormatEx(query, sizeof(query), "SELECT `maptier`, `mapname` FROM `inf_maps`;");
    SQL_TQuery(Influx_GetDB(), DB_OnFetchMapList, query);
}
...
public void DB_OnFetchMapList(Handle owner, Handle hndl, const char[] error, any data)
{
    g_MapList.Clear();

    char map[128], buffer[256];
